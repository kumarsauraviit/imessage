...
export function ChatComposer() {
  ...
  const isMessageSending = useChatStore((state) => state.isMessageSending);
  ...
  // ✅ synchronous lock — independent of React render timing
  ...
  const handleSend = async () => {
    if (isSendingRef.current) return; // ✅ blocks instantly, no render wait
    ...
    <footer className="shrink-0 border-t border-border px-1.5 pb-2 pt-2 sm:px-2">
      ...
      <div className="mx-auto flex w-full max-w-full items-end gap-1.5 px-0.5 sm:gap-2 sm:px-1">
        ...
        <Button
          variant="ghost"
          isIconOnly
          isDisabled={isSendingMedia || isMessageSending}
        ...
        <TextArea
          ...
          onKeyDown={(event) => {
            if (event.key === "Enter" && !event.shiftKey) {
              ...
              handleSend(); // ref guard inside handles repeat/hold-Enter too
            }
          }}
        ...
        <Button
          variant="primary"
          isIconOnly
          isDisabled={!composerText.trim() || isMessageSending}
          onPress={handleSend}
        >
          {isMessageSending ? (
